...
- (void)showPickView:(UIViewController *)vc
{
    ...
    bgView.alpha = 0.3f;
    [vc.view addSubview:bgView];//加一层灰色背景
    
    CGRect frame = self.frame ;
    self.frame = CGRectMake(0,SCREENSIZE.height , SCREENSIZE.width, frame.size.height);//+ frame.size.height
...
- (void)setDataViewWithItem:(NSArray *)items title:(NSString *)title
{
    ...
    titleLbl.text = title;
    titleLbl.textAlignment = NSTextAlignmentCenter;
    titleLbl.textColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.0];//[self getColor:@"FF8000"];
    titleLbl.font = [UIFont fontWithName:@"Helvetica-Bold" size:17.0];
    [header addSubview:titleLbl];
    
    
    UIButton *submit = [[UIButton alloc] initWithFrame:CGRectMake(SCREENSIZE.width - 50, 10 - 4, 50 ,29.5)];
    [submit setTitle:@"完成" forState:UIControlStateNormal];
    [submit setTitleColor:[UIColor colorWithRed:78/255.0 green:175/255.0 blue:239/255.0 alpha:1.0] forState:UIControlStateNormal];
    submit.backgroundColor = [UIColor whiteColor];
    submit.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:15.0];
    [submit addTarget:self action:@selector(submit:) forControlEvents:UIControlEventTouchUpInside];
    [header addSubview:submit];
    
    UIButton *cancel = [[UIButton alloc] initWithFrame:CGRectMake(0, 10 - 4, 50 ,29.5)];
    [cancel setTitle:@"取消" forState:UIControlStateNormal];
    [cancel setTitleColor:[UIColor colorWithRed:78/255.0 green:175/255.0 blue:239/255.0 alpha:1.0] forState:UIControlStateNormal];
    cancel.backgroundColor = [UIColor whiteColor];
    cancel.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:15.0];
    [cancel addTarget:self action:@selector(cancel:) forControlEvents:UIControlEventTouchUpInside];
    [header addSubview:cancel];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    NSLog(@" didSelectRow selectedStr %@",selectedStr);
